...
inline void	optflow_get_vel()	{
	...
	uint16_t local_alt;	// local_alt in	mm*10
	int8_t axis;
	...
	if(
		...
		local_alt	=	constrain((int16_t)alt.EstAlt, 30, 300)	*	min(avgSqual.res,50) * 2;	// 16	bit	ok:	300	*	50 * 2 = 30000;
	}	else {
		local_alt	=	0;
	}
	...
	for(axis=0;axis<2;axis++)	{
		...
		if(local_alt !=	0) { 
			...
			vel_of[axis] = ((int32_t)optflow_pos[axis] + (att.angle[axis]	-	prevAngle[axis]) * 17	)	*	local_alt	/	dTime	;	
		}	else {
			vel_of[axis] = 0;
		}
...
void	Optflow_update() {
	...
	if((f.ANGLE_MODE || f.HORIZON_MODE)	&& optflowMode ==	1
#if GPS
		...
		for(axis=0;	axis<2;	axis++)	{
			optflow_angle[axis]	=	constrain(optflow_angle[axis]	+	(int16_t)((int32_t)optflowErrorI[axis] * conf.pid[PIDVEL].I8	/	5000),
				-300,	300);
		}
